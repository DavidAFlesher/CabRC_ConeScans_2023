...
json_fname = glob.glob(os.path.join('..', 'data', 'plotdensity_data_BChl_2p22.json'))[0]
with open(json_fname, 'r') as f:
    raw_data = json.load(f)

ind_0 = raw_data["A905"]["angle_1"][0].index("0.250") # 0.250
ind_1 = raw_data["A905"]["angle_1"][0].index("2.000")
...
# Plotting
l1 = ax.plot(faux_x_axis, clean_data['A905'][:, 98]*10) # 98=ind of bond distance 1.23
l2 = ax.plot(faux_x_axis, clean_data['A905'][:, 132]*10) # 132=ind of bond distance 1.57
h_l = ax.axhline(alpha=0.5)

# Making things pretty

plt.setp(l1, label = 'C=O', ls = '-', color = "black", alpha = 1, linewidth = 3)
plt.setp(l2, label = 'C-C', ls = "--", color = "black", alpha = 1, linewidth = 3)
...
ax_label.axis('off')
line1 = mlines.Line2D([], [], color = 'black', alpha = 1, ls = '-', linewidth = 3, label = 'C=O, 1.23 Å')
line2 = mlines.Line2D([], [], color = 'black', alpha = 1, ls = '--', linewidth = 3, label = 'C—C, 1.57 Å')
legend = ax_label.legend(handles = [line1, line2], prop = {"size":15}, ncol = 2, loc = 'upper center', bbox_to_anchor = (0.5, 1.15))


# %%

fname = os.path.join("..", "figures", "single_line_bchl2p22_black.png")
