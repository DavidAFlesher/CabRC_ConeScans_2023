...
printf("source CalculateED_lineB2.com %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f 0.0 MapsMtzs/map_${chn}${m}_moved_E PlotDensity/${chn}${m}_cone1_%-2d\n",x0,y0,z0,x1,y1,z1,i-1)}}
eof
awk -f awk.it  PDBs/Cone_${chn}${m}_ref1_axis.pdb >! Part1.com
cat << eof >! awk.it
{x[NR]=0.0+substr(\$0,31,8); y[NR]=0.0+substr(\$0,39,8); z[NR]=0.0+substr(\$0,47,8)}
END{x0=x[1]; y0=y[1]; z0=z[1]; for(i=2; i<=73; i+=1) {x1=x[i]; y1=y[i]; z1=z[i];
printf("source CalculateED_lineB2.com %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f 0.0 MapsMtzs/map_${chn}${m}_moved_E PlotDensity/${chn}${m}_cone2_%-2d\n",x0,y0,z0,x1,y1,z1,i-1)}}
eof
awk -f awk.it  PDBs/Cone_${chn}${m}_ref2_axis.pdb >! Part2.com
cat << eof >! awk.it
{x[NR]=0.0+substr(\$0,31,8); y[NR]=0.0+substr(\$0,39,8); z[NR]=0.0+substr(\$0,47,8)}
END{x0=x[1]; y0=y[1]; z0=z[1]; for(i=2; i<=73; i+=1) {x1=x[i]; y1=y[i]; z1=z[i];
printf("source CalculateED_lineB2.com %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f 0.0 MapsMtzs/map_${chn}${m}_moved_E PlotDensity/${chn}${m}_cone3_%-2d\n",x0,y0,z0,x1,y1,z1,i-1)}}
eof
awk -f awk.it  PDBs/Cone_${chn}${m}_ref3_axis.pdb >! Part3.com
cat << eof >! awk.it
{x[NR]=0.0+substr(\$0,31,8); y[NR]=0.0+substr(\$0,39,8); z[NR]=0.0+substr(\$0,47,8)}
END{x0=x[1]; y0=y[1]; z0=z[1]; for(i=2; i<=73; i+=1) {x1=x[i]; y1=y[i]; z1=z[i];
printf("source CalculateED_lineB2.com %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f 0.0 MapsMtzs/map_${chn}${m}_moved_E PlotDensity/${chn}${m}_cone4_%-2d\n",x0,y0,z0,x1,y1,z1,i-1)}}
eof
